...
// Dados decorativos — agregação temporal real virá na Fase 3
const leadsOverTime = [
...
const funnelData = [
  { stage: "Novo", count: 47 }, { stage: "Em contato", count: 32 },
  { stage: "Proposta", count: 18 }, { stage: "Visita", count: 12 },
...
// Ranking de vendedores decorativo — dados reais virão com módulo de equipe (Fase 3)
...
export default function GarageReports() {
  ...
  // Performance por veículo — dados reais
  const vehiclePerformance = [...veiculos]
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
            ...
            {periods.map((p) => (
              <button
                ...
                className={`rounded-lg px-4 py-1.5 text-micro font-medium transition-colors ${
                  period === p ? "bg-background text-text-primary shadow-card" : "text-text-muted"
                }`}
              >
                {p}
              </button>
          ...
          <button
            disabled
            title="Em breve"
            className="rounded-full border border-border px-4 py-1.5 text-micro font-medium text-text-secondary flex items-center gap-1.5 opacity-50 cursor-not-allowed"
          >
      ...
      {/* Leads over time — decorativo */}
      <div className="rounded-xl border border-border bg-background p-6">
        <h3 className="text-h4 text-text-primary mb-1">Evolução de leads</h3>
        <p className="text-micro text-text-muted mb-4">Dados agregados reais virão na Fase 3.</p>
      ...
      {/* Vehicle performance — REAL */}
      <div className="rounded-xl border border-border bg-background overflow-hidden">
        <div className="p-6 pb-0">
          <h3 className="text-h4 text-text-primary mb-4">Performance por veículo</h3>
        </div>
        {loading ? (
          <div className="p-8 flex justify-center">
            <div className="w-6 h-6 rounded-full border-2 border-garage border-t-transparent animate-spin" />
          </div>
        ) : vehiclePerformance.length === 0 ? (
          <p className="p-6 text-small text-text-muted">Nenhum veículo cadastrado ainda.</p>
        ) : (
      ...
      {/* Vendor performance — decorativo */}
      <div className="rounded-xl border border-border bg-background overflow-hidden">
        <div className="p-6 pb-0">
          <h3 className="text-h4 text-text-primary mb-1">Performance por vendedor</h3>
          <p className="text-micro text-text-muted mb-4">Dados reais disponíveis com módulo de equipe (Fase 3).</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
