...
import { vehicles, garages, formatPrice, formatKm } from "@/data/mock";
import { MapPin, Eye, Heart, Calendar, MessageCircle, Share2, Shield, Sparkles, ChevronRight, Fuel, Gauge } from "lucide-react";
...
export default function VehicleDetailPage() {
  ...
  const garage = vehicle.garagem_id ? garages.find((g) => g.id === vehicle.garagem_id) : null;
  const similar = vehicles.filter((v) => v.id !== vehicle.id).slice(0, 6);
  ...
  const priceTag = {
    abaixo: { label: `▼ ${Math.round(((vehicle.preco_sugerido.max - vehicle.preco) / vehicle.preco_sugerido.max) * 100)}% abaixo da média`, className: "bg-brand-light text-brand-dark" },
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 container mx-auto px-4 lg:px-8 max-w-[1080px]">
        ...
        <div className="text-small text-text-muted mb-4">
          ...
          <span className="hover:text-brand cursor-pointer">Buscar</span>
          <span className="mx-2">›</span>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col md:flex-row gap-2 mb-6">
              <div className="flex-1 relative aspect-video rounded-lg overflow-hidden">
            ...
            <div className="mb-6">
              ...
              <p className="text-body text-text-secondary mt-1">
                {vehicle.ano} · {formatKm(vehicle.quilometragem)} · {vehicle.cambio} · {vehicle.combustivel} · {vehicle.cor}
              </p>
              ...
              <div className="flex items-center gap-4 mt-3 text-text-muted">
                <span className="flex items-center gap-1 text-small"><MapPin className="h-3.5 w-3.5" />{vehicle.cidade}, {vehicle.estado}</span>
                <span className="flex items-center gap-1 text-small"><Calendar className="h-3.5 w-3.5" />{vehicle.data_publicacao}</span>
                <span className="flex items-center gap-1 text-small"><Eye className="h-3.5 w-3.5" />{vehicle.visualizacoes} views</span>
              </div>
            </div>
            ...
            <div className="rounded-lg border border-brand/30 bg-brand-light p-5 mb-8">
              ...
              <div className="flex items-center gap-3 mb-4">
                <span className="font-mono text-body font-medium text-text-primary">Score de confiança: {vehicle.score_confianca}/100</span>
              ...
              {showFinancing && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-4 rounded-lg border border-border bg-surface-card p-5">
                  ...
                  <div className="text-center p-4 rounded-lg bg-surface-secondary">
                    ...
                    <p className="font-display text-3xl font-bold text-text-primary mt-1">{formatPrice(Math.round(parcela))}<span className="text-body font-normal text-text-muted">/mês</span></p>
                ...
                {garage ? (
                  <div className="mt-4 pt-4 border-t border-border">
                    <div className="flex items-center gap-3">
                      <img src={garage.logo_url} alt={garage.nome} className="h-10 w-10 rounded-lg" />
                      <div>
                  ...
                  <div className="mt-4 pt-4 border-t border-border">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-brand-light flex items-center justify-center text-brand-dark font-display font-semibold text-sm">JV</div>
                      <div>
                        <p className="text-small font-medium text-text-primary">Vendedor particular</p>
                        <p className="text-micro text-text-muted">Na plataforma há 2 meses</p>
                      </div>
                    </div>
                  </div>
                )}
