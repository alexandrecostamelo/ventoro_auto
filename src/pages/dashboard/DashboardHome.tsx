...
export default function DashboardHome() {
  ...
  const isLoading = authLoading || veicLoading || leadsLoading;
  if (isLoading) return <SkeletonHome />;

  // ── Derived display data ──────────────────────────────────────────────────

  const nomeSaudacao = profile?.nome?.split(" ")[0] ?? "Usuário";

  const veiculosAtivos = veiculos.filter((v) => v.status === "publicado").slice(0, 3);

  const leadsRecentes = leadsReais.slice(0, 3);

  const alertasDisplay: AlertaDisplay[] = alertas;
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-small text-text-secondary">
                        {v.ano} · {formatarKm(v.quilometragem)} · {v.cambio}
                      </p>
                    </div>
